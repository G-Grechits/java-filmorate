...
public class FilmService {
    ...
    public Film getFilmById(long id) { /*в случае выбрасывания исключений текст в данном случае и ниже начинается
    ...
    public Film updateFilm(Film film) {
        if (!filmStorage.getFilms().containsKey(film.getId())) {
            throw new ObjectNotFoundException(String.format("фильм с ID=%d не существует.", film.getId()));
        }
        if (film.getReleaseDate().isBefore(birthdayOfCinema)) {
            throw new ValidationException("дата релиза фильма не может быть раньше 28 декабря 1895 года.");
        }
